  ...
  async function handleCreateTicket() {
    if (state.inputs.sem > 0) {
      ...
      for (let i = 0; i < state.inputs.sem; i++) {
        ...
        if (response) {
          const ticket = {
            id: uuidv4(), // Unique ID for each ticket
      ...
      // Update state with new tickets without clearing previous tickets
      setState((prev) => ({
        ...prev,
        lotteryCards: [
          ...prev.lotteryCards.filter(card => !createdTickets.some(newCard => newCard.ticketNumber === card.ticketNumber)), // Remove existing cards with the same ticketNumber
          ...createdTickets
        ],
      }));
